...
class BlockCategories extends Module {
    ...
    public function hookAjaxCall($params) {
        try{
            if (!isset($params["idimg"]))
                return;
            $image = new Image($params["idimg"]);
            $image_url = _THEME_PROD_DIR_ . $image->getExistingImgPath() . ".jpg";
            $this->clipImage($image_url, 450, 600);
            exit;
            return "";
        }
        catch(Exception $ex){
            die("primera captura: ".$ex->getMessage());
        }
    }

    function clipImage($file, $width, $height) {
        try{
            $file = str_replace("//", "/", dirname(_PS_ALL_THEMES_DIR_) . $file);
            ...
            die("un error:<br/>".$ex->getMessage());
        }
    }

    /**
     * de momento le voy a poner valores fijos, pero luego le voy a poner parametros
     */
    private function cortarImagen($url, $nw, $nh) {
        $url = str_replace("//", "/", dirname(_PS_ALL_THEMES_DIR_) . $url);
        ...
        imagecopyresampled($tmp, $src_image, 0, 0, $src_x, $src_y, $dst_w, $dst_h, $src_w, $src_h);
