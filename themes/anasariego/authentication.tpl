...
{if !isset($email_create)}
	...
	<form action="{$link->getPageLink('authentication', true)}" method="post" id="create-account_form" class="std">
            <div class="cont_authfs">
		<fieldset class="authfs">
...
<form action="{$link->getPageLink('authentication', true)}" method="post" id="account-creation_form" class="std">
	...
	<fieldset class="account_creation">
            <h3 class="cabeceraseccion">{l s='Your personal information'}</h3>
            <div class="conformsec">
		<p class="radio required">
			<span>{l s='Title'}</span>
			{foreach from=$genders key=k item=gender}
				<input type="radio" name="id_gender" id="id_gender{$gender->id}" value="{$gender->id}" {if isset($smarty.post.id_gender) && $smarty.post.id_gender == $gender->id}checked="checked"{/if} />
	...
	{if $b2b_enable}
	<fieldset class="account_creation">
            <h3 class="cabeceraseccion">{l s='Your company information'}</h3>
            <div class="conformsec">
		<p class="text">
	...
	<fieldset class="account_creation">
            <h3 class="cabeceraseccion">{l s='Your address'}</h3>
            <div class="conformsec">    
		{foreach from=$dlv_all_fields item=field_name}
			{if $field_name eq "company"}
				<p class="text">
	...
	<fieldset class="account_creation dni">
            <h3 class="cabeceraseccion">{l s='Tax identification'}</h3>
            <div class="conformsec">
		<p class="required text">
			<label for="dni">{l s='Identification number'}</label>
			<input type="text" class="text" name="dni" id="dni" value="{if isset($smarty.post.dni)}{$smarty.post.dni}{/if}" />
	...
	<p class="cart_navigation required submit">
		...
		{if isset($back)}<input type="hidden" class="hidden" name="back" value="{$back|escape:'htmlall':'UTF-8'}" />{/if}
		<input type="submit" name="submitAccount" id="submitAccount" value="{l s='Register'}" class="btn" />
		<span><sup>*</sup>{l s='Required field'}</span>
	</p>
</form>
{/if}