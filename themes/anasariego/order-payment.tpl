	...
	<table id="cart_summary" class="std">
		<thead>
			<tr>
				...
				<th class="cart_availability item cabeceratable">{l s='Avail.'}</th>
				<th class="cart_unit item cabeceratable">{l s='Unit price'}</th>
				<th class="cart_quantity item cabeceratable">{l s='Qty'}</th>
				<th class="cart_total last_item cabeceratable">{l s='Total'}</th>
			</tr>
		</thead>
		<tfoot>
			{if $use_taxes}
...
{if !$opc}
	<p class="cart_navigation"><a href="{$link->getPageLink('order', true, NULL, "step=2")}" title="{l s='Previous'}" class="button" id="cart_navigation_continue">&laquo; {l s='Previous'}</a></p>
{else}
	</div>
{/if}
</div>
