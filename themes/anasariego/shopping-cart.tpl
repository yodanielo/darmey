...
{/if*}
<p id="sucestacontiene">{l s='Your shopping cart contains:'} <span id="summary_products_quantity">{$productNumber} {if $productNumber == 1}{l s='product'}{else}{l s='products'}{/if}</span></p>
<div id="order-detail-content" class="table_block">
	<table id="cart_summary" class="std">
		<thead>
			<tr>
				<th class="cart_delete first_item cabeceratable">&nbsp;</th>
				<th class="cart_product cabeceratable">{l s='Product'}</th>
				<th class="cart_description item cabeceratable">{l s='Description'}</th>
				<th class="cart_ref item cabeceratable">{l s='Ref.'}</th>
				<th class="cart_unit item cabeceratable">{l s='Unit price'}</th>
				<th class="cart_quantity item cabeceratable">{l s='Qty'}</th>
				<th class="cart_total item last_item cabeceratable">{l s='Total'}</th>
			</tr>
		</thead>
		<tfoot>
		{if $use_taxes}
		...
		{/if}
			...
			<tr class="cart_total_price">
				...
				<td colspan="2" class="price total_price_container" id="total_price_container">
					<p style="padding: 0px;">{l s='Total:'}</p>
...
<div class="order_delivery clearfix">
	...
	<ul id="delivery_address" class="address item">
		<li class="address_title first_item cabeceratable">{l s='Delivery address'}</li>
	...
	<ul id="invoice_address" class="address alternate_item">
		<li class="address_title last_item cabeceratable">{l s='Invoice address'}</li>
	...
	{else}
		{foreach $formattedAddresses as $address}
			<ul class="address {if $address@last}last_item{elseif $address@first}first_item{/if} {if $address@index % 2}alternate_item{else}item{/if}">
				<li class="address_title cabeceratable">{$address.object.alias}</li>
...
<p class="cart_navigation">
	{if !$opc}
		<a href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}" id="cart_navigation_next" class="exclusive standard-checkout" title="{l s='Next'}">{l s='Next'} &raquo;</a>
		{if Configuration::get('PS_ALLOW_MULTISHIPPING')}
